...
from wakd import datasets
from wakd import models
from wakd.trainers import WAKDTrainer
from wakd.evaluators import Evaluator, extract_features, pairwise_distance
from wakd.utils.data import IterLoader, get_transformer_train, get_transformer_test
from wakd.utils.data.sampler import DistributedRandomDiffTupleSampler, DistributedSliceSampler
from wakd.utils.data.preprocessor import Preprocessor
from wakd.utils.logging import Logger
from wakd.pca import PCA
from wakd.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
from wakd.utils.dist_utils import init_dist, synchronize, convert_sync_bn
from wakd.utils.rerank import re_ranking
...
def get_model(args, type='student'):
    if type == 'student':
        ...
        # base_model = models.create(args.arch, model_path='logs/resnet50-19c8e357.pth')

        pool_layer = models.create('netvlad', dim=base_model.feature_dim)   # or choose nextvlad
        initcache = osp.join(args.init_dir, args.arch + '_' + args.dataset + '_' + str(args.num_clusters) + '_desc_cen.hdf5')

    elif type == 'teacher':
        base_model = models.create(args.archT)
        pool_layer = models.create('netvlad', dim=base_model.feature_dim)
        initcache = osp.join(args.init_dir, args.archT + '_' + args.dataset + '_' + str(args.num_clusters) + '_desc_cen.hdf5')
    else:
...
def main_worker(args):
    ...
    model_teacher = get_model(args, type='teacher')
    checkpoint_teacher = load_checkpoint(args.resumeT)
    copy_state_dict(checkpoint_teacher['state_dict'], model_teacher)
    ...
    # Trainer
    trainer = WAKDTrainer(model, model_cache, model_teacher, margin=args.margin**0.5,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="WAKD training")
    ...
    parser.add_argument('-b', '--archT', type=str, default='vgg16',
                        choices=models.names())
    ...
    parser.add_argument('--resumeT', type=str, default='', metavar='PATH')
